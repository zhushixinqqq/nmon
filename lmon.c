...
float percent_cpu = 0.0;   /* percent of the processes CPU interval by add zhushixin 2017-02*/
...
int main(int argc, char **argv)
{
	...
	char temp_buf[1024];//total path
	char buf_first[64]={0};//first
	char buf_second[64]={0};//
	char buf_third[64]={0};//
...
/* External Data Collector Controls */
    ...
    while (-1 !=
	   (i =
	    getopt(argc, argv,
		   "?abc:C:Dd:EfF:g:hI:l:m:MNpr:Rs:tTUVxXzy:"))) {
	switch (i) {
	case '?':
	    hint();
	    exit(0);
...
#define LSBLK_NO_TYPE		/* define this to work around missing --output TYPE feature */
	...
	case 'z':		/* background mode for 1 day output to /var/perf/tmp */
	    ...
	    go_background(4 * 24, 15 * 60);
	    break;
	case 'y':		/* background mode for 1 day output to /var/perf/tmp */
	    percent_cpu=atof(optarg);
	    break;
	}
    }
	...
	topper_temp_size = n;
    topper_temp = MALLOC(sizeof(struct topper_temp) * topper_temp_size);
    /* Get Disk Stats. */
    proc_disk(0.0);
...
*/
	...
	if (show_top) {
		...
		topper_temp = REALLOC(topper_temp, sizeof(struct topper_temp) * (p->processes +1));/* add one to avoid overrun */
		topper_temp_size = p->processes;
	    }
		...
		for (j = 0; j < q->processes; j++) {
			...
			topper_temp[max_sorted].index=i;
			topper_temp[max_sorted].process_path[0]="";
			//strncpy(topper_temp[max_sorted].process_path,p->procs[i].pi_comm,sizeof(p->procs[i].pi_comm));
			//
		...
		case 5:
		    ...
		    for (j = 0; j < max_sorted; j++) {
			i = topper[j].index;
			get_cmdline_from_pid(p->procs[i].pi_pid,temp_buf,1024);
			sscanf(temp_buf,"%s%s%s",buf_first,buf_second,buf_third);
...
*/
				...
				mvwprintw(padtop, j + 3 - skipped, (COLS >119)?97:61, "%-32s", p->procs[i].pi_comm);
			        COLOUR wattrset(padtop, COLOR_PAIR(0));
				mvwprintw(padtop, j + 3 - skipped, (COLS >119)?120:100, "%-32s%-32s", buf_first,buf_second);
			        COLOUR wattrset(padtop, COLOR_PAIR(0));
			    }
			} else {
			    if ((cmdfound && cmdcheck(p->procs[i].pi_comm))
				...
				if((topper[j].time / elapsed)>percent_cpu){
